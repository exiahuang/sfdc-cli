...
def register(parser, subparsers, **kwargs):
    ...
    def handler(args):
        if args.project and args.dir and args.name:
            ...
            if args.unzip:
                retrieve_api.unzip(os.path.join(args.dir,
                                                args.name), args.project,
                                   args.srcdir, args.delete_after_unzip)
        else:
    ...
    subcommand.add_argument('-s',
                            '--srcdir',
                            type=str,
                            help='src relative path, default src. if use sfdx, please set it force-app/main/default',
                            default="src")
